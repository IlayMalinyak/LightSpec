...
from nn.models import CNNEncoder, MultiEncoder, CNNEncoderDecoder, CNNRegressor, MultiTaskRegressor, MultiTaskSimSiam
# from nn.mamba import MambaEncoder
from nn.simsiam import SimSiam
from nn.train import ContrastiveTrainer, MaskedRegressorTrainer, ContrastiveRegressorTrainer
...
if data_args.masked_transform:
    ...
    trainer = ContrastiveRegressorTrainer(model=model, optimizer=optimizer,
                       ...
                       scheduler=scheduler, train_dataloader=train_dataloader, num_quantiles=len(optim_args.quantiles),
                            val_dataloader=val_dataloader, device=local_rank, ssl_weight=1,
                           exp_num=datetime_dir, log_path=data_args.log_dir, range_update=None,
                           accumulation_step=1, max_iter=np.inf,
                        exp_name=f"{model_name}_lc_{exp_num}")
...
preds_val, targets_val, info_val = trainer.predict(val_dataloader, device=local_rank)
