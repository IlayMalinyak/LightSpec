...
class MLA(nn.Module):
    ...
    def reset_cache(self):
        """Reset the cached tensors between batches"""
        if attn_impl == "naive":
            self.k_cache.zero_()
            self.v_cache.zero_()
        else:
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        if args.time_series:
            ...
            self.embed = nn.Linear(args.input_features, args.dim, bias=False)
        else:
            print("language transformer!")
    ...
    def reset_caches(self):
        """Reset all attention caches in the model"""
        for layer in self.layers:
            layer.attn.reset_cache()

    def forward(self, tokens: torch.Tensor, y: torch.Tensor = None, start_pos: int = 0):
        self.reset_caches()
        ...
        # Handle dimensionality without in-place operations
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    args = ModelArgs()
    x = torch.randint(0, args.vocab_size, (2, 1, 54))
