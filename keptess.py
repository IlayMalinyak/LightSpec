...
def create_train_test_dfs(meta_columns):
    lc_df = pd.read_csv('/data/lightPred/tables/kepler_dr25_tess_valid.csv')
    berger_df = pd.read_csv('/data/lightPred/tables/berger_catalog_full.csv')
    lc_df = lc_df.merge(berger_df, on='KID', how='left')
    lc_df['kmag_abs'] = lc_df['KMAG'] - 5 * np.log10(lc_df['Dist']) + 5
    ...
    kepler_df = priority_merge_prot(p_dfs, lc_df)

    print("total number of sampes: ", len(lc_df), " kepler samples: ", len(lc_df[lc_df['KID'].notna()]),
    " TESS samples: ", len(lc_df[lc_df['TID'].notna()]))
...
def create_raw_samples(data_args, num_samples=10):
    ...
    for i in range(num_samples):
        ...
        print(info['lightcurve_type'], lc.shape)
        plt.plot(t, lc)
        plt.xlabel('Time (days)')
        plt.ylabel('Flux')
        plt.title(f"{info['lightcurve_type']} ID: {info['ID']}")
        plt.savefig(f'/data/lightSpec/images/raw_keptess_{i}.png')
        plt.close()
...
for i in range(100):
    ...
    if y.isnan().any():
        nans += 1
    print(lc.shape, target_lc.shape, y, info['lightcurve_type'])
    if i % 10 == 0:
        time = lc[0]
        flux = lc[1]
        acf = lc[2]
        fig, axes = plt.subplots(1,2)
        axes[0].plot(time, flux)
        axes[0].set_title('flux')
        axes[1].plot(time, acf)
        axes[1].set_title('ACF')
        fig.suptitle(f"{info['lightcurve_type']} {info['ID']}")
        plt.savefig(f'/data/lightSpec/images/keptess_{i}.png')
        plt.close()
print(nans)
...
trainer = ContrastiveTrainer(model=model, optimizer=optimizer,
                    ...
                    val_dataloader=val_dataloader, device=local_rank, ssl_weight=0,
                        ...
                        accumulation_step=1, max_iter=np.inf, add_time=data_args.add_time,
                        exp_name=f"keptess_{exp_num}") 
...
config_save_path = f"{data_args.log_dir}/{datetime_dir}/keptess_{exp_num}_complete_config.yaml"
...
output_filename = f'{data_args.log_dir}/{datetime_dir}/{model_name}_keptess_{exp_num}.json'
with open(output_filename, "w") as f:
    json.dump(fit_res, f, indent=2)
fig, axes = plot_fit(fit_res, legend=exp_num, train_test_overlay=True)
plt.savefig(f"{data_args.log_dir}/{datetime_dir}/fit_{model_name}_keptess_{exp_num}.png")
plt.clf()

predict_results(trainer, val_dataloader, test_dataloader, loss_fn,
                 data_args.prediction_labels_lc,
                 data_args, optim_args, 'keptess', exp_num,
